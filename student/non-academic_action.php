...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $s_nas = trim($_POST['s_nas']);      
    $s_basic = trim($_POST['s_basic']);    
    $s_skills = trim($_POST['s_skills']); 
    $s_work = trim($_POST['s_work']);
    ...
    if ($_FILES) {
        $requirements = [
            ...
            'moral_file' => 'moral_file_path_non_acad',
            'grades_file' => 'grades_file_non_acad',
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $conn->prepare("
                INSERT INTO applications (scholarship_application_id, s_nas, s_basic, s_skills, s_work)
                VALUES (?, ?, ?, ?, ?)
                ON DUPLICATE KEY UPDATE 
                    s_nas = VALUES(s_nas),
                    s_basic = VALUES(s_basic),
                    s_skills = VALUES(s_skills),
                    s_work = VALUES(s_work)
            ");
            $stmt->bind_param("issss", $scholarship_application_id, $s_nas, $s_basic, $s_skills, $s_work);
