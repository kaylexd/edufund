        ...
        case 'update_status':
            if (isset($_POST['s_id']) && isset($_POST['status'])) {
                if ($_POST['status'] === 'Approve') {
                    $stmt = $conn->prepare("UPDATE admin_status SET s_scholar_status = 'Approved', is_scholar = 1 WHERE student_id = ?");
                } else {
                    $stmt = $conn->prepare("UPDATE admin_status SET s_scholar_status = 'Rejected', is_scholar = 0 WHERE student_id = ?");
                }
